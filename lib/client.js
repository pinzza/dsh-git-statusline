window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 这里优先读绑定源，旧版本（0.1.5）则回退到 `list.current`，两边都能工作。
    function useCurrentSessionId(ctx, list) {
      ...
      return fromBinding ?? list.current
    }
    ...
    function GitStatusline({ wide }) {
      ...
      const sessionId = useCurrentSessionId(ctx, list)
